...
# cv2.imshow("sharpening", shrap_img)
...
for key_color, color in upper.items():
    ...
    mask_open = opening(mask, STREL)
    # cv2.imshow("opening", mask_open.copy())
    # melakukan morfologi CLOSING
    mask_close = closing(mask_open, STREL)
    # cv2.imshow("closing", mask_close.copy())

    """
    Setelah setiap warna di masking, munculkan hasilnya
    """
    if key_color == "matang" :
        # cv2.imshow("matang", mask_close)
        pass
    elif key_color == "mentah":
        # cv2.imshow("mentah", mask_close)
        pass
    elif key_color == "setengah matang":
        # cv2.imshow("setengah matang", mask_close)
        pass
    ...
    cnts, _ = cv2.findContours(mask_close.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
